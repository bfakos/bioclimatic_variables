...
# Supporting Information S4 of Bede-Fazekas, Á., & Somodi, I. (2020). The way bioclimatic variables are calculated has impact on potential distribution models. Methods in Ecology and Evolution. https://doi.org/10.1111/2041-210X.13488
# Modification (02 Dec 2020): L55 is added
...
calculate_BCVs <- function(databases){
	...
	for (database_name in database_names) { # iterate through the databases
		...
		for (temporal_approach in temporal_approaches) { # iterate through the one or two temporal approaches
			...
			rownames(database) <- NULL # remove row names (which could shuffle the results)
			...
			database$bcv5 <- as.numeric(database[cbind(c(1:row_number), Tmax_columns[months_and_quarters[[database_name_for_selection]][, "warmestM"]])]) # BCV5 = Max Temperature of Warmest Month
			database$bcv6 <- as.numeric(database[cbind(c(1:row_number), Tmin_columns[months_and_quarters[[database_name_for_selection]][, "coldestM"]])]) # BCV6 = Min Temperature of Coldest Month
			...
			values1 <- as.numeric(database[cbind(c(1:row_number), Tmean_columns[indices_of_first_month])]) # values of the first month of the quarter
			values2 <- as.numeric(database[cbind(c(1:row_number), Tmean_columns[indices_of_first_month %% 12 + 1])]) # values of the second month of the quarter
			values3 <- as.numeric(database[cbind(c(1:row_number), Tmean_columns[(indices_of_first_month + 1) %% 12 + 1])]) # values of the third month of the quarter
			...
			database$bcv13 <- as.numeric(database[cbind(c(1:row_number), P_columns[months_and_quarters[[database_name_for_selection]][, "wettestM"]])]) # BCV13 = Precipitation of Wettest Month
			database$bcv14 <- as.numeric(database[cbind(c(1:row_number), P_columns[months_and_quarters[[database_name_for_selection]][, "driestM"]])]) # BCV14 = Precipitation of Driest Month
			...
			indices_of_first_month <- months_and_quarters[[database_name_for_selection]][, "wettestQ"] # get the indices of the first month of the quarter
			values1 <- as.numeric(database[cbind(c(1:row_number), P_columns[indices_of_first_month])]) # values of the first month of the quarter
			values2 <- as.numeric(database[cbind(c(1:row_number), P_columns[indices_of_first_month %% 12 + 1])]) # values of the second month of the quarter
			values3 <- as.numeric(database[cbind(c(1:row_number), P_columns[(indices_of_first_month + 1) %% 12 + 1])]) # values of the third month of the quarter
