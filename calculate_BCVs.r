...
#                 all data.frames have the same structure (i.e. number of rows, order of the rows), which is similar to that of the input databases
...
calculate_BCVs <- function(databases){
	...
	for (database_name in database_names) { # iterate through the databases
		months_and_quarters[[database_name]] <- data.frame(matrix(data = NA, ncol = length(specific_period_names), nrow = row_number)) # add an empty data.frame as a new list element
